...
#ifndef BM_LOG_CLOSE
#define BM_LOG_CLOSE() bm_utilities::Logger::destroy()
#endif
...
public:
    ...
    static void init(const std::string &dir, const std::string &fname) {
        bm_utilities::Logger::log_instance = new Logger(dir, fname);
    }
    ...
    static void destroy() {
        if (bm_utilities::Logger::log_instance != NULL) {
            delete bm_utilities::Logger::log_instance;
            bm_utilities::Logger::log_instance = NULL;
        }
    }

public:
    static Logger *log_instance; // 静态日志对象实例
